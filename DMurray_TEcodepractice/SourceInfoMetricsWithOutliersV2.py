...
These metrics are not normalized. Normalization can be done using the function:
...
def ComputeInofoTheoreticMetricsAndSignificance(Yt, Pt, nbins, numiter, alpha, shift, MaxLag, ncore,lowerSink, upperSink, lowerSource, upperSource,ZFlagSink, ZFlagSource, LagDependentCrit):
...
#     Qt_1 = copy.deepcopy(Yt[:-1])
    ...
    # Do the one time computations of critical values based on maxLag
    #===============================================
    ...
    if LagDependentCrit == 1:
        MIcrit = np.nan*np.ones([MaxLag])
        TEcrit = np.nan*np.ones([MaxLag])
        CorrCrit = np.nan*np.ones([MaxLag])
        criticalTotMI = np.nan*np.ones([MaxLag])
        criticalTotMI_Qt_1 = np.nan*np.ones([MaxLag])
        MIcritP = np.nan*np.ones([MaxLag])
        TEcritP = np.nan*np.ones([MaxLag])
        CorrCritP = np.nan*np.ones([MaxLag]) 
    
    for lag in np.arange(MaxLag):

        # MI and TE conditioned on Qt-1       
        Minput = LagData_new( M_unlagged=np.column_stack((Pt,Yt,Yt)), shift=[-lag,shift[1],shift[2]]) #  MIcrit, TEcrit, CorrCrit : 3
        ...
        ### Total Info
            # based on Pt-1
        MinputTot = LagData_new( M_unlagged=np.column_stack((Yt,Pt,Pt)), shift=[0,0,-lag]) #  criticalTotMI : 2
        ...
        HQt, HXt, HXt_1, IQtXtXt_1[lag] = ComputeTotalEntropy(NTot,nbins)
            # based on Qt-1
        MTotQt_1 = LagData_new( M_unlagged=np.column_stack((Yt,Pt,Yt)), shift=[0,-lag,-1]) # criticalTotMI_Qt_1 : 1
        ...
        MinputP = LagData_new( M_unlagged=np.column_stack((Pt,Yt,Pt)), shift=[-lag,shift[1],shift[2]]) # MIcritP, TEcritP, CorrCritP : 4
        ...
        # ===============================
        # Lag dependent critical values
        if LagDependentCrit == 1:
            
            MIcrit[lag], TEcrit[lag], CorrCrit[lag] = computeCritical_TE_MI_Corr(Minput, nbins, numiter, alpha,ncore,lower_bound3, upper_bound3,ZFlagSource,ZFlagSink,ZFlagSink) # 3
            
            criticalTotMI[lag] = computeCritical_Total_MI(MinputTot, nbins, numiter, alpha, ncore,lower_bound2, upper_bound2,ZFlagSink, ZFlagSource,ZFlagSource)
            
            criticalTotMI_Qt_1[lag] = computeCritical_Total_MI(MTotQt_1, nbins, numiter, alpha, ncore,lower_bound1, upper_bound1, ZFlagSink, ZFlagSource, ZFlagSink)
            
            MIcritP[lag], TEcritP[lag], CorrCritP[lag] = computeCritical_TE_MI_Corr(MinputP, nbins, numiter, alpha,ncore,lower_bound4, upper_bound4, ZFlagSource,ZFlagSink,ZFlagSource)
